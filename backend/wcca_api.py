...
def _friendly_error_message(raw: str) -> str:
    """Map upstream errors to user-friendly Chinese messages."""
    msg = (raw or "").strip()
    low = msg.lower()

    if "budget has been exceeded" in low or "max budget" in low:
        return "API 调用额度已用尽，请更换可用账号或提升预算后重试。"
    if "unauthorized" in low or "invalid api key" in low or "forbidden" in low:
        return "API 鉴权失败，请检查 token 或账号权限。"
    return msg or "服务暂时不可用，请稍后重试。"
...
async def wcca_session_start(user_id: str = Form(...)):
    ...
    try:
        ...
        session_id = await loop.run_in_executor(
            None, agent_runner.start_wcca_session, user_id, workdir
        )
    except Exception as e:
        detail = _friendly_error_message(str(e))
...
async def wcca_run_agent(payload: dict):
    ...
    if not result.get("ok"):
        ...
        raw_error = str(result.get("error", "agent failed"))

        return {

            "ok": False,

            "error": _friendly_error_message(raw_error),
